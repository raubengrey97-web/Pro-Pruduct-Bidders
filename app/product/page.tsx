...
export default function Products() {
  ...
  const placeBid = async (product: any) => {
    ...
    if (error) {
      setBidMessages({ ...bidMessages, [product.id]: 'Error placing bid: ' + error.message })
    } else {
  ...
  return (
    <>
      ...
      <main style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto' }}>
        <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '32px', fontWeight: 700, color: '#111', marginBottom: '8px' }}>
          Live Auctions
        </h1>
        <p style={{ color: '#888', marginBottom: '40px' }}>Browse active products and place your bids</p>
            ...
            {products.map(p => (
              <div key={p.id} style={{ background: '#fff', border: '1px solid #ececec', borderRadius: '12px', overflow: 'hidden' }}>
                {p.photo_url ? (
                  ...
                  <div style={{ width: '100%', height: '180px', background: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ccc', fontSize: '13px' }}>
                    No image
                  </div>
                )}
